    ...
    def Q_learning(self, discount=1.0, alpha=0.01, max_seq_len=500,
        ...
        for episode in range(TRAINING_EPOCH_NUM):
            ...
            while not (done or timeout):
                ...
                # ***********   Q value update   ****************
                i, j = state  # 현재 state를 저장
                i_next, j_next = next_state  # 다음 state를 저장
                print(episode)
                print(self.Q_values[i_next, j_next, :])
                self.Q_values[i, j, action] += alpha * (reward + discount * np.max(self.Q_values[i_next, j_next, :]) - self.Q_values[i, j, action])
                # Q value를 업데이트
